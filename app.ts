...
4. AESTHETICS: Use Markdown (headings, bold, lists) ONLY when helpful for technical/complex content. Keep simple answers clean and plain.`;
    ...
    if (isStreamRequested) {
      ...
      let userErrMsg = `**সীমা অতিক্রম (All Keys Rate Limited):** সিস্টেমে উপলব্ধ মোট ${shuffledKeys.length}টি Naga API Key-এর প্রতিটিরই সীমা শেষ হয়েছে। দয়া করে নতুন Naga API Key যুক্ত করুন।`;

      const errData = JSON.stringify({
        id: `chatcmpl-err-${Date.now()}`,
        object: "chat.completion.chunk",
        created: Math.floor(Date.now() / 1000),
        model: modelFromClient || "nemotron-3-ultra-550b-a55b:free",
        choices: [{
          index: 0,
          delta: { content: `\n\n${userErrMsg}` },
          finish_reason: "stop"
...
app.use((req, res, next) => {
  ...
  if (path.includes("/models/")) {
    const requestedId = path.split("/models/")[1] || "nemotron-3-ultra-550b-a55b:free";
    return res.json({
      id: requestedId,
      object: "model",
      created: Math.floor(Date.now() / 1000),
